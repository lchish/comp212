...
if(isset($_GET['id'])){
  ...
  header("Location: index.php?debug=no id");
 }
//echo $clean_auction_number;
//exit();
...
if(mysql_num_rows($result) > 0){
  ...
  header("Location: index.php?debug=no rows");
  exit();
 }

if($_SESSION['username'] != $auction_creater){
  header("Location: index.php?debug=wrong creator");
  exit();
 }
...
?>
  <p>Upload an image to go with <a href="auction.php?id=<?php echo $clean_auction_number;?>">your auction</a>:</p>
...
<br>
<input type="hidden" value="<?php echo $clean_auction_number;?>" 
name="auctionnumber">
